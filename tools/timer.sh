...
INHIBIT_PID=""
if command -v python3 &>/dev/null && python3 -c "import gi" &>/dev/null 2>&1; then
    python3 - <<'PYEOF' &
...
cookie = proxy.call_sync(
    'Inhibit',
    GLib.Variant('(susu)', ('timer.sh', 0, 'Timer running', 8)),
...
signal.signal(signal.SIGTERM, _uninhibit)
signal.signal(signal.SIGINT, _uninhibit)
GLib.MainLoop().run()
PYEOF
    INHIBIT_PID=$!
fi
